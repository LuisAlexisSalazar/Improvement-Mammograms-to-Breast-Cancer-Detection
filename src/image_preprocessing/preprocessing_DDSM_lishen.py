...
class DMImagePreprocessor(object):
    ...
    def remove_pectoral(self, img, breast_mask, high_int_threshold=.8,
        ...
        # !Remover objetos que se confunan con el musculo pectroal
        w_b, h_i = img_bin.shape
        # temp_img_bin = copy(img_bin)
        # print(img_bin.shape)
        img_bin[w_b // 2:] = 0
        # print(img_bin.shape)
        # cv2.imshow("img_bin", cv2.resize(img_bin, (224, 224)))
        # cv2.imshow("temp_img_bin", cv2.resize(temp_img_bin, (224, 224)))
        # cv2.waitKey()

        pect_marker_img = np.zeros(img_bin.shape, dtype=np.int32)
        # pect_marker_img = np.zeros(img_bin.shape, dtype=np.uint8)
        # Sure foreground (shall be pectoral).
        pect_mask_init = self.select_largest_obj(img_bin, lab_val=maxval,
                                                 fill_holes=False,
                                                 # fill_holes=True,
                                                 smooth_boundary=False)
        # cv2.imshow("pect_mask_init", cv2.resize(pect_mask_init, (224, 224)))
        # cv2.waitKey()

        kernel_ = np.ones((morph_kn_size, morph_kn_size), dtype=np.uint8)
        # kernel_ = np.ones((5, 5), dtype=np.uint8)
        pect_mask_eroded = cv2.erode(pect_mask_init, kernel_,
                                     iterations=n_morph_op)

        # print(pect_mask_eroded.dtype)
        # cv2.imshow("pect_mask_eroded", cv2.resize(pect_mask_eroded, (224, 224)))
        # cv2.waitKey()
        # print(np.unique(pect_mask_eroded))
        # pect_marker_img[pect_mask_eroded > 0] = 255
        pect_marker_img[pect_mask_eroded > 0] = 255
        # print("New Type(No convierte ):", pect_marker_img.dtype)
        # cv2.imshow("pect_marker_img", cv2.resize(pect_marker_img, (224, 224)))
        # cv2.waitKey()
        ...
        # cv2.imshow("img_breast_only", cv2.resize(img_breast_only, (224, 224)))
        # cv2.waitKey()
    ...
    def only_remove_pectoral_muscle(self, img_proc, mask_, high_int_threshold=.8, **pect_kwargs):
        img_proc_copy = copy(img_proc)
        # img_proc = cv2.equalizeHist(img_proc)
        # _, mask_ = self.suppress_artifacts(img_proc)

        # pect_high_inten_thres = 200  # <<= para to tune!
        # _, pect_binary_thres = cv2.threshold(img_proc, pect_high_inten_thres,
        ...
        img_proc_copy = cv2.resize(img_proc_copy, (224, 224))
        mask_ = cv2.resize(mask_, (224, 224))
        img_proc = cv2.resize(img_proc, (224, 224))
        img_col = cv2.resize(img_col, (224, 224))
        # pect_binary_thres = cv2.resize(pect_binary_thres, (224, 224))
        # pect_mask_watershed = cv2.resize(pect_mask_watershed, (224, 224))
        # mammo_breast_equ_3c = cv2.resize(mammo_breast_equ_3c, (224, 224))

        cv2.imshow('img_proc_copy', img_proc_copy)
        # cv2.imshow('mask_', mask_)
        cv2.imshow('img_proc', img_proc)
        # cv2.imshow('img_col', img_col)
        # cv2.imshow('pect_binary_thres', pect_binary_thres)
        # cv2.imshow('pect_mask_watershed', pect_mask_watershed)
        # cv2.imshow('mammo_breast_equ_3c', mammo_breast_equ_3c)
        cv2.waitKey()
